...
function doOffer(data) {
	...
	var tokenSecret = data.token.attributes.tokenSecret;
  ...
  console.log(config);
	request.post({
		url: url + '/session',
		json: {
			callbackUrl: config.host + ':' + config.port + '/session/',
			to: toTN,
			from: fromTN,
			fromDisplay: 'rtcgateway'
		},
		oauth: {
			consumer_key: config.webrtcgw.consumerKey,
			consumer_secret: config.webrtcgw.consumerSecret,
			token: token,
	...
	function(err, res, data) {
		if (err) {
			console.log('problem connecting to gateway');
			...
			if (sessions[target].candidates.length > 0) doCandidates(sessions[target].candidates);
			request.put({
				url: url + '/session/' + uuid,
				json: offerData
			},
			function(err, res, data) {
				if (err) {
...
function doCandidate(data) {
	...
	if (sessions[target]) {
		if (sessions[target].active == true) {
			request.put({
				url: url + '/session/' + sessions[target].uuid,
				json: data
			},
			function(err, res, data) {
...
module.exports.EndPoint = function(ExpressInstance, conf) {
	url = conf.jsep2sipgw || "http://127.0.0.1:8080";
	app = ExpressInstance;
	config = conf;
}

module.exports.createConnector = function() {
